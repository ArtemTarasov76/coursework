...
class RegistrationWindow(QWidget):
    ...
    def register_user(self):
        ...
        # Проверяем, был ли выбран файл фотографии
        if not self.photo_path:
            # Устанавливаем фотографию по умолчанию
            default_photo_path = "templates/default.png"  # Путь к фотографии по умолчанию
...
class ProfileWidget(QWidget):
    def __init__(self, user, current_user_id):
        ...
        if pixmap.isNull():  # Проверяем, загрузилось ли изображение
            print(f"Фотография не загружена: {user[5]}")
            pixmap = QPixmap("templates/default.png")  # Используем фото по умолчанию
